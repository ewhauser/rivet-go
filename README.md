...
The current release is `v0.2.0` and targets Rivet Engine `v2.3.10`.

## Install

```sh
go get github.com/ewhauser/rivet-go/rivet@v0.2.0
```
...
```sh
go run github.com/ewhauser/rivet-go/cmd/rivet-go-fetch@v0.2.0
```
...
| Actor-scoped background work | Supported | `Actor.Run` restarts with each generation and serializes actor-state access while queue waits and `RunContext.KeepAwake` yield the actor turn. `Context.KeepAwake` and restricted detached `WaitUntil` work are registered with core, reliably ended across transient native backpressure, fenced when teardown begins, and drained during shutdown. |
