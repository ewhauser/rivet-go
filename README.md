...
| Actor-scoped background work | Supported | `Actor.Run` restarts with each generation and serializes actor-state access while queue waits and `RunContext.KeepAwake` yield the actor turn. `Context.KeepAwake` and restricted detached `WaitUntil` work are registered with core, reliably ended across transient native backpressure, and drained during shutdown. |
