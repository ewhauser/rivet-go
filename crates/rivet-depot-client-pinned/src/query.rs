...
fn column_value(stmt: *mut libsqlite3_sys::sqlite3_stmt, index: i32) -> ColumnValue {
	match unsafe { sqlite3_column_type(stmt, index) } {
		...
		SQLITE_BLOB => {
			...
			// sqlite3_column_type already established SQLITE_BLOB. SQLite may
			// return a null data pointer for a valid zero-length blob; preserve
			// that type instead of collapsing it to SQL NULL.
			if blob_len == 0 {
